        ...
        if isinstance(res, list):
            ...
            for m in recent:
                ...
                for row in m.get("statistics", []):
                    ...
                    if row.get("type") == "Corners": s["corners"] += val
                s["cnt"] += 1
...
with tab1:
    ...
    st.caption("Engine stitches together the highest-confidence mathematical picks.")
    ...
    if st.button("Generate Institutional Slip"):
        if isinstance(daily_matches, list):
            ...
            if len(final_slip) < pick_count:
                st.warning(f"Engine is strict: Not enough high-confidence games today to safely build a {target_odds} slip. Here are the top plays instead:")
...
with tab2:
    ...
    if isinstance(weekly_matches, list):
        ...
        for d in dates[:4]: # Show next 4 days to prevent lag
            st.markdown(f"#### 📅 {d}")
